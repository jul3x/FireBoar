    ...
    switch (name) {
      case "play":
        ...
        try {
          ...
          // Android Doze mode can release audio codec resources after ~2 min
          // of screen lock. Re-set the source to force re-initialization.
          debugPrint("Audio.play failed ($e), re-initializing source and retrying");
          if (_src != null) {
            var assetSrc = control.backend.getAssetSource(_src!);
            if (assetSrc.isFile) {
              await player.setSourceDeviceFile(assetSrc.path);
